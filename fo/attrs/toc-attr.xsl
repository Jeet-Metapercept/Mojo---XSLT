...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:attribute-set name="__toc__topic__content">
        ...
        <xsl:attribute name="font-size">
            <xsl:variable name="level" select="count(ancestor-or-self::*[contains(@class, ' topic/topic ')])"/>
            <xsl:choose>
                <xsl:when test="$level = 1"><!--11pt-->11pt</xsl:when>
                <xsl:otherwise><!--<xsl:value-of select="$default-font-size"/>-->11pt</xsl:otherwise>
            </xsl:choose>
        </xsl:attribute>
        <xsl:attribute name="font-weight">
            <xsl:variable name="level" select="count(ancestor-or-self::*[contains(@class, ' topic/topic ')])"/>
            <xsl:choose>
                <xsl:when test="$level = 1"><!--bold-->normal</xsl:when>
    ...
    <xsl:attribute-set name="__toc__chapter__content" use-attribute-sets="__toc__topic__content">
        <xsl:attribute name="font-family">FontMojo</xsl:attribute>
        <xsl:attribute name="font-size"><!--13pt-->11pt</xsl:attribute>
        <!--<xsl:attribute name="font-weight">bold</xsl:attribute>-->
